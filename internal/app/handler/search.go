...
func (s *Setup) SearchRecommendation(w http.ResponseWriter, r *http.Request) {
	params := r.URL.Query()
	if len(params) == 0 {
		helper.DecodeError(w, nil, errQueryField, http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		helper.DecodeError(w, err, errSearch, http.StatusInternalServerError)
		return
	}
...
func (s *Setup) SearchGenre(w http.ResponseWriter, r *http.Request) {
	params := r.URL.Query()
	if len(params) == 0 {
		helper.DecodeError(w, nil, "The query field is required", http.StatusBadRequest)
		return
	}
	if err := s.v.Var(params["query"][0], "required"); err != nil {
		helper.SearchValidatorMessage(w)
	...
	if err != nil {
		helper.DecodeError(w, err, "Could not do the search", http.StatusInternalServerError)
		return
	}
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(&search)
